...
#include <rtthread.h>
#include <rtdevice.h>
#include <RadioDataProcess/Radio_Encoder.h>
#include <stdio.h>
#include <string.h>
#include "led.h"
#include "drv_spi.h"
...
uint32_t Self_Id = 0;
uint32_t Self_Default_Id = 10000999;
...
{
    char *buf = rt_malloc(64);
    sprintf(buf, "-B{%02d,%08ld,%08ld,%08ld,%03d,%03d,%02d}B", ack, Gateway_ID, Self_Id, device_id, rssi, warn_id,
            value);
    Normal_send(buf, 45);
    rt_free(buf);
}
...
{
    char *buf = rt_malloc(64);
    sprintf(buf, "-C{%02d,%08ld,%08ld,%08ld,%03d,%03d,%02d}C", ack, Gateway_ID, Self_Id, device_id, rssi, control,
            value);
    Normal_send(buf, 45);
    rt_free(buf);
}
...
void RadioDequeue(void *paramaeter)
{
    while (1)
    {
        ...
        if (Main_Queue.TargetNum > Main_Queue.NowNum)
        {
            ...
            {
            case 1:
                RadioSend(Main_Queue.Taget_Id[Main_Queue.SendNum], Main_Queue.counter[Main_Queue.SendNum],
                        Main_Queue.Command[Main_Queue.SendNum], Main_Queue.Data[Main_Queue.SendNum]);
                LOG_D("Normal Send With Now Num %d,Target Num is %d,Target_Id %ld,counter %d,command %d,data %d\r\n",
                        Main_Queue.SendNum, Main_Queue.TargetNum, Main_Queue.Taget_Id[Main_Queue.SendNum],
                        Main_Queue.counter[Main_Queue.SendNum], Main_Queue.Command[Main_Queue.SendNum],
                        Main_Queue.Data[Main_Queue.SendNum]);
                rt_thread_mdelay(100);
                break;
            case 2:
                rt_thread_mdelay(200);
                GatewaySyncSend(Main_Queue.ack[Main_Queue.SendNum], Main_Queue.counter[Main_Queue.SendNum],
                        Main_Queue.Taget_Id[Main_Queue.SendNum], Main_Queue.Command[Main_Queue.SendNum],
                        Main_Queue.Data[Main_Queue.SendNum]);
                LOG_D("GatewaySync With Now Num %d,Target Num is %d,Type is %d,Target_Id %ld,rssi %d,bat %d\r\n",
                        Main_Queue.SendNum, Main_Queue.TargetNum, Main_Queue.counter[Main_Queue.SendNum], Gateway_ID,
                        Main_Queue.Command[Main_Queue.SendNum], Main_Queue.Data[Main_Queue.SendNum]);
            ...
            case 3:
                rt_thread_mdelay(200);
                GatewayWarningSend(Main_Queue.ack[Main_Queue.SendNum], Main_Queue.Taget_Id[Main_Queue.SendNum],
                        Main_Queue.counter[Main_Queue.SendNum], Main_Queue.Command[Main_Queue.SendNum],
                        Main_Queue.Data[Main_Queue.SendNum]);
                LOG_D(
                        "GatewayWarningSend With Now Num %d,Target Num is %d,Target_Id %ld,Rssi is %d,warn_id %d,value %d\r\n",
                        Main_Queue.SendNum, Main_Queue.TargetNum, Gateway_ID, Main_Queue.counter[Main_Queue.SendNum],
                        Main_Queue.Command[Main_Queue.SendNum], Main_Queue.Data[Main_Queue.SendNum]);
                rt_thread_mdelay(300);
                wifi_led(3);
                break;
            case 4:
                rt_thread_mdelay(200);
                GatewayControlSend(Main_Queue.ack[Main_Queue.SendNum], Main_Queue.Taget_Id[Main_Queue.SendNum],
                        Main_Queue.counter[Main_Queue.SendNum], Main_Queue.Command[Main_Queue.SendNum],
                        Main_Queue.Data[Main_Queue.SendNum]);
                LOG_D(
                        "GatewayControl With Now Num %d,Target Num is %d,Target_Id %ld,Rssi is %d,control %d,value %d\r\n",
                        Main_Queue.SendNum, Main_Queue.TargetNum, Gateway_ID, Main_Queue.counter[Main_Queue.SendNum],
                        Main_Queue.Command[Main_Queue.SendNum], Main_Queue.Data[Main_Queue.SendNum]);
                rt_thread_mdelay(300);
                wifi_led(3);
...
void RadioDequeueTaskInit(void)
{
    ...
    if (Radio_QueueTask)
        rt_thread_startup(Radio_QueueTask);
}
